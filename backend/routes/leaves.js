...
module.exports = function (db, notify) {
  ...
  router.put('/:id', async (req, res) => {
    try {
      ...
      const lr = await db.get('SELECT * FROM leave_requests WHERE id=?', req.params.id);
      ...
      `, leave_type, start_date, end_date, parseFloat(days), reason || null, status, req.params.id);
      const staffRow = await db.get('SELECT name FROM staff WHERE id=?', lr.staff_id);
      await db.run('INSERT INTO audit_log (role, action, details) VALUES (?,?,?)', role, 'Leave Record Edited', `${staffRow?.name} | ${leave_type} | ${start_date}~${end_date}`);
  ...
  router.delete('/:id', async (req, res) => {
    try {
      ...
      const staffRow = await db.get('SELECT name FROM staff WHERE id=?', lr.staff_id);
      await db.run('INSERT INTO audit_log (role, action, details) VALUES (?,?,?)', role, 'Leave Record Deleted', `${staffRow?.name} | ${lr.leave_type} | ${lr.start_date}~${lr.end_date}`);
      res.json({ success: true });
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  ...
  router.get('/:id/document', async (req, res) => {
    try {
      const lr = await db.get('SELECT document_data, document_mime, document_name FROM leave_requests WHERE id=?', req.params.id);
  ...
  router.post('/', upload.single('document'), async (req, res) => {
    try {
      ...
      const result = await db.run(`
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?,'pending',?)
      `, staff_id, leave_type, start_date, end_date, days, reason || null, document_data, document_mime, document_name, half_day_period, now);
      ...
      await db.run('INSERT INTO audit_log (role, action, details) VALUES (?,?,?)', role, 'Leave Applied', details);
  ...
  router.put('/:id/approve', async (req, res) => {
    try {
      ...
      const role = req.headers['x-user-role'] || 'unknown';
      const now = new Date().toISOString();
      await db.run(`UPDATE leave_requests SET status='approved', decided_at=?, director_notes=? WHERE id=?`,
        now, director_notes || null, req.params.id);

      const lr = await db.get(`SELECT lr.*, s.name FROM leave_requests lr JOIN staff s ON s.id=lr.staff_id WHERE lr.id=?`, req.params.id);
      const details = lr ? `${lr.name} | ${lr.leave_type} | ${lr.start_date} to ${lr.end_date}` : `Leave #${req.params.id}`;
      await db.run('INSERT INTO audit_log (role, action, details) VALUES (?,?,?)', role, 'Leave Approved', details);
  ...
  router.put('/:id/reject', async (req, res) => {
    try {
      ...
      const role = req.headers['x-user-role'] || 'unknown';
      const now = new Date().toISOString();
      await db.run(`UPDATE leave_requests SET status='rejected', decided_at=?, director_notes=? WHERE id=?`,
        now, director_notes || null, req.params.id);

      const lr = await db.get(`SELECT lr.*, s.name FROM leave_requests lr JOIN staff s ON s.id=lr.staff_id WHERE lr.id=?`, req.params.id);
      const details = lr ? `${lr.name} | ${lr.leave_type} | ${lr.start_date} to ${lr.end_date}` : `Leave #${req.params.id}`;
      await db.run('INSERT INTO audit_log (role, action, details) VALUES (?,?,?)', role, 'Leave Rejected', details);
