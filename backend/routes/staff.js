...
module.exports = function (db, notify) {
  ...
  router.put('/leave-opening', async (req, res) => {
    try {
      ...
      for (const u of updates) {
        await stmt.run(u.annual_opening_used ?? 0, u.mc_opening_used ?? 0, u.id);
      }

      await db.prepare('INSERT INTO audit_log (role, action, details) VALUES (?,?,?)').run(role, 'Leave Opening Balance Updated', `${updates.length} staff updated`);
      if (role === 'hr') {
  ...
  router.put('/:id', async (req, res) => {
    try {
      const { name, ic_number, email, phone, department, job_title, date_joined, annual_entitlement, mc_entitlement, staff_pin } = req.body;
      const role = req.headers['x-user-role'] || 'unknown';

      const before = await db.prepare('SELECT name FROM staff WHERE id=?').get(req.params.id);
      await db.prepare(`
        UPDATE staff SET name=?, ic_number=?, email=?, phone=?,
          department=?, job_title=?, date_joined=?, annual_entitlement=?, mc_entitlement=?, staff_pin=?
        WHERE id=?
      `).run(name, ic_number, email, phone, department, job_title, date_joined, annual_entitlement, mc_entitlement, staff_pin || null, req.params.id);

      const details = `${before ? before.name : `Staff #${req.params.id}`} | ${job_title} | ${department}`;
      await db.prepare('INSERT INTO audit_log (role, action, details) VALUES (?,?,?)').run(role, 'Staff Profile Edited', details);
